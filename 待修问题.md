...
### 11. Chunk 缓存写入部分失败 [低优先级] - 待优化
**发现日期**: 2026-06-21  
**状态**: ⚠️ 不影响下载，仅优化失败  

**问题描述**:
部分 xorb 在写入 chunk 缓存时因长度不匹配而失败。
...
**问题分析**:
- `fetch_infos` 的 `chunk_range` 合并后期望 156 个 chunks
- 但 xorb 实际只包含 92 个 chunks（93 个 indices）
- 可能原因：xorb 只包含文件的部分 chunks，fetch_infos 描述的范围超出了 xorb 的实际内容

**当前解决方案** (commit: 待提交):
- 添加详细的调试日志，输出 fetch_infos 详情
- 跳过长度不匹配的情况，避免错误写入
- 不影响下载功能（仅缓存优化失败）

**代码位置**: `xet/pipeline/chunk_cache_adapter.py:124-165`

**修复方案（待实现）**:
1. **方案 A**: 参考 Rust 原版，理解 fetch_infos 的真实语义
2. **方案 B**: 为每个 fetch_info 分别缓存（而非合并）
3. **方案 C**: 只缓存完整的 xorb（fetch_infos 完全覆盖 xorb 内容）

**影响评估**:
- ✅ 下载功能正常
- ⚠️ 部分 xorb 无法缓存到 chunk-level（回退到 xorb-level 缓存）
- ⚠️ 缓存命中率可能略低（但仍有 xorb-level 缓存兜底）

**后续工作**:
- [ ] 查看完整的调试日志，分析实际的 fetch_infos 结构
- [ ] 参考 `~/xet` Rust 原版的实现逻辑
- [ ] 确定正确的缓存粒度（per-xorb vs per-fetch_info）
